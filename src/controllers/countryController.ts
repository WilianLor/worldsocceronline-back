...
export default {
  async create(req: Request, res: Response) {
    const {
      ...
      pictureUrl,
      countryRegionId,
      mainNacionalCompetitionName,
      mainNacionalCompetitionnumberOfTeams,
      secondaryNacionalCompetitionName,
      secondaryNacionalCompetitionnumberOfTeams,
      pictureUrlMain,
      pictureUrlSecondary,
    } = req.body;
    const mainNacionalCompetitionData = {
      name: mainNacionalCompetitionName,
      numberOfTeams: mainNacionalCompetitionnumberOfTeams,
      countryId: "",
      pictureUrl: pictureUrlMain,
    };
    const secondaryNacionalCompetitionData = {
      name: secondaryNacionalCompetitionName,
      numberOfTeams: secondaryNacionalCompetitionnumberOfTeams,
      countryId: "",
      pictureUrl: pictureUrlSecondary,
    };
    ...
    try {
      ...
      const mainNacionalCompetition = await RunningPointsCompetition.create(
        mainNacionalCompetitionData
      );
      const secondaryNacionalCompetition =
        await RunningPointsCompetition.create(secondaryNacionalCompetitionData);
  ...
  async getCountries(req: Request, res: Response) {
    ...
    try {
      ...
      const countries = await Country.find({ regionId: regionId })
        .select("-regionId")
        .select("-mainNacionalCompetitionId")
        .select("-secondaryNacionalCompetitionId")
        .select("-__v");
  ...
  async getLeagues(req: Request, res: Response) {
    ...
    try {
      ...
      const leagues = await RunningPointsCompetition.find({
        countryId: countryId,
      })
        .select("-numberOfTeams")
        .select("-qualifyForMainRegionalCompetition")
        .select("-qualifyForSecondaryRegionalCompetition")
        .select("-winnerId")
        .select("-award")
        .select("-teams")
        .select("-__v")
